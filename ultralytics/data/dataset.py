...
class YOLORGBTDataset(BaseDataset):
    """Dataset class for loading object detection and/or segmentation labels in YOLO format.
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        self.data = data
        assert not (self.use_segments and self.use_keypoints), "Can not use both segments and keypoints."
        assert self.data.get("channels", 4) == 4, "For RGBT dataset, the channel number must be 4."
        super().__init__(*args, channels=4, **kwargs)
    ...
    def get_labels(self) -> list[dict]:
        ...
        Returns:
            (list[dict]): List of label dictionaries, each containing information about an image and its annotations.
        """
        self.label_files = img2label_paths(self.im_files, RGBT=True)
        cache_path = Path(self.label_files[0]).parent.with_suffix(".cache")
        try:
            cache, exists = load_dataset_cache_file(cache_path), True  # attempt to load a *.cache file
            assert cache["version"] == DATASET_CACHE_VERSION  # matches current version
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        f_ir = f.replace("visible", "infrared")
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    im = cv2.imread(f, flags=cv2.IMREAD_COLOR)  # BGR
                    im_ir = cv2.imread(f_ir, flags=cv2.IMREAD_GRAYSCALE)
            else:  # read image
                im = cv2.imread(f, flags=cv2.IMREAD_COLOR)  # BGR
                im_ir = cv2.imread(f_ir, flags=cv2.IMREAD_GRAYSCALE)
            if im is None:

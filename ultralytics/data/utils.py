...
def img2label_paths(img_paths: list[str], RGBT=False) -> list[str]:
    """Convert image paths to label paths by replacing 'images' with 'labels' and extension with '.txt'."""
    if RGBT:
        sa, sb = f"{os.sep}visible{os.sep}images{os.sep}", f"{os.sep}infrared{os.sep}labels{os.sep}"  # /visible/images/, /infrared/labels/ substrings
    else:
